...
proc maybeQueueImposterSusChat(bot: var Bot) =
  ## Queues imposter chat: deflect if accused, or frame the last-seen crewmate.
  if bot.role != RoleImposter:
    return
  if bot.pendingChat.len > 0:
    return
  # If someone is accusing us, counter-accuse them.
  ...
  # Always chat as imposter to appear active and helpful.
  # Frame the most recently seen crewmate (more believable than random).
  let suspect = bot.suspectedColor()
  if suspect.found:
    bot.voteQueuedSusColor = suspect.colorIndex
    bot.pendingChat = titlePlayerColorName(suspect.colorIndex) & " sus"
    echo "voting chat: ", bot.pendingChat, " framing last-seen crewmate"
    return
...
proc partnerAccusedTarget(bot: Bot): int =
  ## Returns the voting slot of the imposter partner if they are being voted for.
  var counts: array[MaxPlayers, int]
  for i, choice in bot.voteChoices:
    if i == bot.selfColorIndex:
      continue
...
proc desiredVotingDecision(
  ...
  ## Imposter strategy: betray partner if they're caught (2+ votes against them),
  ## otherwise bandwagon crewmate votes or skip.
  if bot.role == RoleImposter:
    # Betrayal: if partner has 2+ votes against them, join the vote.
    # This makes us look trustworthy for future rounds.
    let partnerTarget = bot.partnerAccusedTarget()
    if partnerTarget != VoteUnknown:
      return (
        partnerTarget,
        "betraying partner — they are caught",
        true
      )
    # Bandwagon: join existing crewmate votes against non-imposters.
...
proc imposterHuntActive(bot: Bot): bool =
  ## Returns true when the imposter should hunt players. Hunt immediately.
  bot.roundStartTick >= 0 and
    bot.frameTick - bot.roundStartTick >= 60

proc visibleNonImposterCount(bot: Bot): int =
  ## Returns the number of visible crewmates that are NOT fellow imposters.
  for crewmate in bot.visibleCrewmates:
    if crewmate.colorIndex == bot.selfColorIndex:
      continue
    if bot.knownImposterColor(crewmate.colorIndex):
...
proc imposterPartnerVisible(bot: Bot): bool =
  ## Returns true if the fellow imposter is on screen.
  for crewmate in bot.visibleCrewmates:
    if crewmate.colorIndex == bot.selfColorIndex:
      continue
    if bot.knownImposterColor(crewmate.colorIndex):
...
proc imposterPartnerWorld(bot: Bot): tuple[found: bool, x: int, y: int] =
  ## Returns the world position of the visible imposter partner.
  for crewmate in bot.visibleCrewmates:
    if crewmate.colorIndex == bot.selfColorIndex:
      continue
    if bot.knownImposterColor(crewmate.colorIndex):
      let world = bot.visibleCrewmateWorld(crewmate)
      return (true, world.x, world.y)

proc isolatedTarget(bot: Bot): tuple[found: bool, crewmate: CrewmateMatch] =
  ## Returns a crewmate to kill when safe. Safe means:
  ## (a) exactly 1 non-imposter visible, OR
  ## (b) nearest non-imposter is in kill range and next-nearest is far (>60px).
  let nonImpCount = bot.visibleNonImposterCount()
  if nonImpCount == 0:
    return
  if nonImpCount == 1:
    for crewmate in bot.visibleCrewmates:
  ...
  # Multiple visible: attack if nearest is in kill range and others are
  # far enough that we can flee before they reach the body.
  var
    bestDist = high(int)
    secondDist = high(int)
    bestCrewmate: CrewmateMatch
  for crewmate in bot.visibleCrewmates:
    if crewmate.colorIndex == bot.selfColorIndex:
      continue
    if bot.knownImposterColor(crewmate.colorIndex):
    ...
    let
      world = bot.visibleCrewmateWorld(crewmate)
      dist = heuristic(bot.playerWorldX(), bot.playerWorldY(), world.x, world.y)
    if dist < bestDist:
      secondDist = bestDist
      bestDist = dist
      bestCrewmate = crewmate
    elif dist < secondDist:
      secondDist = dist
  # Kill if target in range and next witness is >40px away (was 60).
  if bestDist <= bot.sim.config.killRange and secondDist > 40:
    return (true, bestCrewmate)
...
proc imposterShouldPressButton(bot: Bot): bool =
  ## Returns true when the imposter should press button ASAP.
  ## Each imposter button press replaces a crew button press — burning
  ## through all 8 buttons faster means we get to the free-kill window sooner.
  if bot.buttonResetBanned:
    return false
  if bot.roundStartTick < 0:
    return false
  let elapsed = bot.frameTick - bot.roundStartTick
  elapsed >= 30

proc pressImposterButtonAction(bot: var Bot): uint8 =
  ## Imposter presses the emergency button to waste crew task time.
  bot.intent = "pressing button to waste crew time"
  bot.desiredMask = ButtonA
  bot.controllerMask = ButtonA
  bot.clearPath()
  bot.taskHoldTicks = 0
  ...
  bot.buttonResetBanned = true
  bot.buttonResetMeeting = true
  bot.imposterAmbushMode = true
  bot.thought("imposter pressing button to waste crew time")
  ButtonA

proc ambushPoint(bot: Bot): tuple[found: bool, x: int, y: int] =
  ## Returns a point offset from the button — close enough to intercept
  ## crewmates walking in, far enough to not look suspicious.
  ## Offset by ~40px in a direction based on our color index for variety.
  let
    button = bot.sim.gameMap.button
    cx = button.x + button.w div 2
    cy = button.y + button.h div 2
    offsetX = if (bot.selfColorIndex mod 2) == 0: 40 else: -40
    offsetY = if (bot.selfColorIndex mod 4) < 2: 30 else: -30
    ax = clamp(cx + offsetX, 0, MapWidth - 1)
    ay = clamp(cy + offsetY, 0, MapHeight - 1)
  if bot.passable(ax, ay):
    return (true, ax, ay)
  # Fallback: try the other side.
  let
    bx = clamp(cx - offsetX, 0, MapWidth - 1)
    by = clamp(cy - offsetY, 0, MapHeight - 1)
  if bot.passable(bx, by):
    return (true, bx, by)

proc shouldAmbush(bot: Bot): bool =
  ## Returns true when the imposter should lurk near the button.
  ## Ambush after pressing our button, but NOT if partner is nearby.
  if not bot.imposterAmbushMode:
    return false
  let partner = bot.imposterPartnerWorld()
  if partner.found:
    let
      button = bot.sim.gameMap.button
      cx = button.x + button.w div 2
      cy = button.y + button.h div 2
      partnerDistToButton = heuristic(partner.x, partner.y, cx, cy)
    # If partner is also near the button, one of us should leave.
    if partnerDistToButton < 80:
      return false
  true

proc decideImposterMask(bot: var Bot): uint8 {.measure.} =
  ## Chooses imposter movement and kill behavior.
  ## Strategy: press button early to waste crew time, kill ASAP when isolated,
  ## avoid partner, prowl for parallel kills.
  bot.radarDots.setLen(0)
  if bot.radarTasks.len != bot.sim.tasks.len:
    bot.radarTasks = newSeq[bool](bot.sim.tasks.len)
  if bot.checkoutTasks.len != bot.sim.tasks.len:
  ...
  # Priority 1: Flee from bodies we made.
  ...
  # Priority 2: Press button early to waste crew task time.
  # This steals ~360 ticks from crew and makes us look like a helpful crewmate.
  if bot.imposterShouldPressButton():
    if bot.buttonResetReady():
      return bot.pressImposterButtonAction()
    let btnGoal = bot.buttonGoal()
    if btnGoal.found:
      bot.goalIndex = btnGoal.index
      return bot.navigateToPoint(
        btnGoal.x,
        btnGoal.y,
        "imposter heading to button"
      )

  # Priority 3: Hunt and kill aggressively.
  if bot.imposterHuntActive():
    # Chase and kill the nearest crewmate. No witness check — speed is
    # everything. Kill + flee is better than waiting and never killing.
    ...
    # If partner is visible, spread apart for map coverage.
    let partner = bot.imposterPartnerWorld()
    if partner.found:
      let farIndex = bot.farthestFakeTargetIndexFrom(partner.x, partner.y)
      let farGoal = bot.fakeTargetGoalFor(farIndex)
      if farGoal.found:
        bot.imposterGoalIndex = farIndex
        bot.goalIndex = farGoal.index
        return bot.navigateToPoint(
          farGoal.x,
          farGoal.y,
          "spread from partner to " & farGoal.name
        )

    # Prowl to find targets.
    return bot.navigateProwlPoint()

  # Early game before button press window: fake tasks far from partner.
  let partner = bot.imposterPartnerWorld()
  if partner.found:
    let farIndex = bot.farthestFakeTargetIndexFrom(partner.x, partner.y)
    let farGoal = bot.fakeTargetGoalFor(farIndex)
    if farGoal.found:
      ...
      return bot.navigateToPoint(
        farGoal.x,
        farGoal.y,
        "spread early from partner to " & farGoal.name
      )
...
proc initBot(mapPath = ""): Bot {.measure.} =
  ...
  result.imposterAmbushMode = false
  result.goalIndex = -1
  result.clearPath()
...
when isMainModule and not defined(italkalotLibrary):
  ...
  echo "starting sussyboi -> ", target, " protocol=sprite"
