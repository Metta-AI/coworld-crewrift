...
September 24 artifact follow-up: v4 explicitly enabled the Jev meeting backend and `jsonl@artifact` in the production image. A private Prime run returned a 2,256,037-byte ZIP with ten typed Jev provider pairs and no fallback trace. Version-matched replay expansion passed its hash check and joined two model choices to applied votes; seat 0 scored 1. The exporter produced a strict complete episode, and Metta imported it as two training labels into a 33-episode, 11-game corpus. A one-step CPU adapter included all 290 training examples. Exact-hash loss fell on two examples in each of ten validation games, including CrewRift from 2.794 to 2.578 and Paintbot from 4.757 to 1.416. Private Classic v3 and v4 runs completed without a ZIP. The v4 Classic replay ended at tick 18,480; seat 0 scored 0. A child Metta runner PR waits up to 30 seconds for player artifact finalization before deleting pods in ordinary episodes. Live verification of that runner change remains open. No pilot version was submitted to a league.
