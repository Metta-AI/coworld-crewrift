# Smart Bot for Crewrift — Architecture & Build Plan

This guide describes how to build an LLM-augmented policy for the uploaded
Crewrift Coworld, layering memory, strategic reasoning, and cross-game
learning on top of a visual player.

The core insight: the existing `notsus.nim` source bot already solves the
...
Your policy connects to that websocket, reads Sprite v1 frames, and sends
Sprite v1 button or chat packets. The runner owns slot assignment and token
...
3. submit it to a Crewrift league;
...
<https://github.com/Metta-AI/coworld-crewrift/blob/master/docs/sprite_v1.md>; the
global replay/viewer protocol is
<https://github.com/Metta-AI/coworld-crewrift/blob/master/docs/sprite_v1.md>; and the
browser play guide is <https://github.com/Metta-AI/coworld-crewrift/blob/master/docs/play_crewrift.md>.
...
The architecture below is still useful without a Crewrift checkout: treat
`notsus.nim` as a reference design for perception, localization, task
...
- Receives Sprite v1 updates over WebSocket
- Maintains object, label, layer, and viewport state from the sprite stream
...
The CvC debugger policy uses a three-tier memory system that maps cleanly onto Crewrift.
...
CvC system use "halls" (categories). For Crewrift:
...
The brain sits between perception (notsus's existing pipeline) and output
...
Handles deterministic per-frame execution. This is what notsus.nim already
...
```
You are an AI player in Crewrift, a social deduction game.
...
- `notsus.nim` handles perception, navigation, physics (fast path)
...
├── notsus/             # Optional vendored source/reference perception stack
├── sidecar/
│   ├── __init__.py
...
1. **Build a valid Coworld player image.** The process must read
   `COGAMES_ENGINE_WS_URL`, connect to it, and send valid Sprite input.
2. **Choose the perception base.** Either port the `notsus.nim` ideas into
